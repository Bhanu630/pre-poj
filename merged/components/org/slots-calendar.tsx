...
import { Plus, X, Clock, CalendarDays, Trash2, Pencil } from "lucide-react"
...
import { addSlot, deleteSlot, getMySlots, updateSlot, type SlotDoc } from "@/lib/firestore"
...
export function SlotsCalendar() {
  ...
  function openAdd() {
    setEditingSlot(null)
    setForm({ ...defaultForm(), date: selectedDate ?? todayIso() })
    setDialogOpen(true)
  }
  ...
  function openEdit(slot: SlotDoc) {
    ...
    setForm({
      ...
      amountRequired: slot.amountRequired ? String(slot.amountRequired) : "",
  ...
  async function handleSave() {
    ...
    const sponsored = editingSlot?.sponsored ?? 0
                  ...
                  <div key={slot.id} className="rounded-xl border border-gray-100 bg-white p-4 shadow-sm">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0 flex-1">
                        ...
                        {slot.amountRequired && (
                          <p className="mt-0.5 text-xs text-muted-foreground">
                            ₹{slot.amountRequired.toLocaleString("en-IN")} required
                          </p>
                        )}
                        {slot.description && (
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="rounded-2xl sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              {editingSlot ? "Update the details for this slot." : "Create a new meal slot for donors to sponsor."}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="flex flex-col gap-4 py-2">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="flex flex-col gap-1.5">
                <Label htmlFor="total-meals">Total Meals Needed</Label>
                <Input
                  id="total-meals"
                  type="number"
                  min={0}
              ...
              <div className="flex flex-col gap-1.5">
                <Label htmlFor="amount-required">Amount Required (₹)</Label>
                <Input
                  id="amount-required"
                  type="number"
                  min={0}
                  value={form.amountRequired}
                  onChange={(e) => setForm((f) => ({ ...f, amountRequired: e.target.value }))}
                  placeholder="3500"
                  className="rounded-xl"
                />
              </div>
