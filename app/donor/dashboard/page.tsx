      ...
      <section className="grid grid-cols-1 gap-6 lg:grid-cols-3">
                ...
                <li key={activity.id} className="flex flex-wrap items-center gap-4 py-4 first:pt-0 last:pb-0">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-sm text-muted-foreground"> 
                      {activity.organizationName || 'Organization'} &middot; {activity.createdAt?.toDate().toLocaleDateString() || "Today"}
                    </p>
                  </div>
        ...
        <div className="rounded-2xl border border-border bg-card p-6 shadow-sm">
          ...
          <div className="flex flex-col gap-4">
            {recommended.map((org) => (
              <div key={org.uid} className="rounded-xl border border-border p-4 transition-colors hover:border-primary/40">
                ...
                <Button asChild size="sm" className="mt-3 w-full rounded-lg"> 
                  <Link href={`/donor/org/${org.uid}`}>Sponsor Now</Link>
                </Button>
              </div>
            ))}
          </div>
